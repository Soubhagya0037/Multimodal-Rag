...
from transformers import AutoModelForCausalLM, MultiModalityCausalLM
import base64
...
def load_model_with_retry(model_class, model_name, max_retries=5, **kwargs):
    """Load model with exponential backoff retry logic"""
    for attempt in range(max_retries):
        try:
            print(f"Attempting to load {model_name} (attempt {attempt + 1}/{max_retries})")
            model = model_class.from_pretrained(model_name, **kwargs)
            print(f"Successfully loaded {model_name}")
            return model
        except Exception as e:
            if ("429" in str(e) or "rate limit" in str(e).lower()) and attempt < max_retries - 1:
                wait_time = (2 ** attempt) + (torch.rand(1).item() * 2)  # Add jitter
                print(f"Rate limited loading {model_name}, waiting {wait_time:.1f}s before retry")
                time.sleep(wait_time)
            elif attempt < max_retries - 1:
                wait_time = 2 ** attempt
                print(f"Error loading {model_name}: {str(e)[:100]}... Retrying in {wait_time}s")
                time.sleep(wait_time)
            else:
                print(f"Failed to load {model_name} after {max_retries} attempts")
                raise e
...
class EmbedData:
    ...
    def _load_embed_model(self):
        # Load ColPali model with retry logic
        embed_model = load_model_with_retry(
            ColPali,
        ...
        processor = load_model_with_retry(
            ColPaliProcessor,
            self.embed_model_name
        )
...
class QdrantVDB_QB:
    ...
    def define_client(self):
        self.client = QdrantClient(url="http://localhost:6334", prefer_grpc=True)
...
class RAG:
    ...
    def _setup_llm(self):
        # Load VLChatProcessor with retry logic
        self.vl_chat_processor = load_model_with_retry(
            VLChatProcessor,
            self.llm_name,
            cache_dir="./Janus/hf_cache"
        )
        ...
        # Load AutoModelForCausalLM with retry logic
